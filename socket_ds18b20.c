...
*/
        ...
        while(retry_count<MAX_RETRY_TIMES)
        {
                ...
                if(sockfd<0)
                {
                        log_write(LOG_LEVEL_ERROR,"recreate socket failure:%s\n",strerror(errno));
                        return -1;
                }
                memset(&servaddr,0,sizeof(servaddr));
                servaddr.sin_family=AF_INET;
                ...
                {
                        log_write(LOG_LEVEL_ERROR,"connect errno:%s\n",strerror(errno));
                        //exit(EXIT_FAILURE);
                }

                log_write(LOG_LEVEL_INFO,"Retry after %ds...\n",RETRY_INTERVAL);
        ...
        log_write(LOG_LEVEL_INFO,"connect to server[%s,%d] successfully\n",servip,port);
